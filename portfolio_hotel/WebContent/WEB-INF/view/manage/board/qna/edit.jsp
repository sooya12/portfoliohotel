				...
				<div class="con">
					...
					<div id="bbs">
						<div id="bread">
							...
							<table width="100%" border="0" cellspacing="0" cellpadding="0" summary="관리자 관리 기본내용입니다.">
								...
								<tbody>
									...
									<tr>
										...
										<td >
											<input type="text" id="email" name="email" class="w50" title="이메일을 입력해주세요" value="<%=Function.checkNull(data.getEmail())%>" />	
											
										</td>
										<th scope="row"><label for="">게시글 비밀번호</label></th>
										<td colspan="3" >
											<input type="text" id="password" name="password" class="w20" title="비밀번호를 입력해주세요" value="<%=Function.checkNull(data.getPassword())%>" />	
									...
									<tr>
										...
										<td colspan="4">
											<textarea id="contents" name="contents" title="내용을 입력해주세요" style="width:100%;" value="<%=Function.checkNull(data.getContents())%>" ></textarea>	
										</td>
									</tr>
									<tr>		<!-- 서버에서 사용하는 파일이름 / 한글을 사용하지 않기위해 org에  / 파일기능 첨부는 post로  form 에는  enctype 사용해야함. -->
										...
										<td colspan="3">
											...
											<% } else { %>
												<div class="weidtFile">
													<p>기존파일 : <a href="<%= Function.downloadUrl(SiteProperty.NOTICE_UPLOAD_PATH, java.net.URLEncoder.encode(data.getFile_org(), "UTF-8"), data.getFile()) %>" target="_blank"><%= Function.checkNull(data.getFile_org()) %> [<%= Function.getFileSize(data.getFilesize())%>]</a><br />
										...
										<input type="hidden" name="stype" id="stype" value="<%=data.getStype()%>"/>
								<input type="hidden" name="sval" id="sval" value="<%=data.getSval()%>"/>
							...
							<div class="btn">
								<div class="btnLeft">
									<a class="btns" href="<%=data.getTargetURLParam("index", data, 0)%>"><strong>목록</strong></a>
								</div>
								<div class="btnRight">
