...
<%
    ...
    if ( req_tx.equals( "pay" ) )
    {
            ...
            c_PayPlus.mf_set_us( ordr_data_set_no, "ordr_mony", request.getParameter("total_price") );
    ...
    /* = -------------------------------------------------------------------------- = */
        if ( res_cd.equals( "0000" ) )
        {
            ...
            if ( use_pay_method.equals( "100000000000" ) )
            {
                ...
                rvo.setRoom_pk(Integer.parseInt(request.getParameter("room_pk")));
                rvo.setRoom_name(request.getParameter("room_name"));
                
                if(request.getParameter("member_pk") != null) {
                	rvo.setMember_pk(Integer.parseInt(request.getParameter("member_pk")));
                }
                
                rvo.setCheckin(request.getParameter("checkin"));
                rvo.setCheckout(request.getParameter("checkout"));
                rvo.setAdult(Integer.parseInt(request.getParameter("adult")));
                rvo.setKid(Integer.parseInt(request.getParameter("kid")));
                rvo.setRoom_price(Integer.parseInt(request.getParameter("room_price")));
                rvo.setPerson_price(Integer.parseInt(request.getParameter("person_price")));
                rvo.setOption_price(Integer.parseInt(request.getParameter("option_price")));
                rvo.setCharge_price(Integer.parseInt(request.getParameter("charge_price")));
                rvo.setTotal_price(Integer.parseInt(request.getParameter("total_price")));
                rvo.setGuest_lastname(request.getParameter("guest_lastname"));
                rvo.setGuest_firstname(request.getParameter("guest_firstname"));
                rvo.setGuest_tel1(request.getParameter("guest_tel1"));
                rvo.setGuest_tel2(request.getParameter("guest_tel2"));
                rvo.setGuest_tel3(request.getParameter("guest_tel3"));
                rvo.setGuest_email(request.getParameter("guest_email"));
                rvo.setPay_method(1);
                rvo.setPay_state(1);
                rvo.setPaydate(DateUtil.getToday());
				...
				Room_opt_resVO ovo = new Room_opt_resVO();
				ovo.setRoom_res_pk(r);
				for(int i=0; i<option_pk.length; i++) {
					ovo.setOption_pk(Integer.parseInt(option_pk[i]));
					ovo.setCount(Integer.parseInt(count[i]));
					ovo.setPrice(Integer.parseInt(price[i]));
					ovo.setName(name[i]);
					ovo.setTotal_price(ovo.getPrice() * ovo.getCount());
					dao.insert_opt(ovo);
				}
            }
...
<html xmlns="http://www.w3.org/1999/xhtml" >
    ...
    <form name="pay_info" method="post" action="/book/room/confirm_room">
