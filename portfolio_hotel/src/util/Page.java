...
//			strList.append(" <a href='"+listUrl+"reqPageNo="+pageCount+"' CLASS='b_num'>["+pageCount+"]</a>");
	...
	public static String userIndexList(int reqPageNo, int pageCount, HttpServletRequest req) {
		...
		strList.append("<div class='table-page'><ul class=\"page-number clear\">");
		// 첫번째 페이지 인덱스 화면이 아닌경우
		if ( reqPageNo > pagenumber) {
			curpage = startpage - 1;    // 시작페이지 번호보다 1 적은 페이지로 이동
			strList.append("<li><a href='"+listUrl+"reqPageNo="+curpage+"' class='next'>[이전]</a></li>");
//			strList.append("<a href='"+listUrl+"reqPageNo=1' CLASS='b_num'>[1]</a> ");
		}else{
			strList.append(" ");
		}
		...
		while (curpage <= endpage){
			if (curpage == reqPageNo) {
			  strList.append("<li class='on'><a href='#'>"+reqPageNo+"</a></li>");
			} else {
		...
		if ( pageCount > endpage) {
			curpage = endpage + 1;  
			strList.append("<li><a href='"+listUrl+"reqPageNo="+curpage+"' class='next'>다음</a></li>");
//			strList.append(" <a href='"+listUrl+"reqPageNo="+pageCount+"' CLASS='b_num'>["+pageCount+"]</a>");
		}else{
			strList.append(" ");
		}
		strList.append("</ul></div> ");
